...
class Kunou {
  ...
  val ownerId = sys.env("KUNOU_OWNER_ID").toLong
  val defaultCommandPrefix = sys.env.get("KUNOU_DEFAULT_PREFIX") match {
    case None => "k."
  ...
  private[this] val catnipOptions = new CatnipOptions(token)
    ...
    .validateToken(true)
  val catnip = Catnip.catnip(catnipOptions).connect()
